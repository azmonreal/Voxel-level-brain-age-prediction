...
from monai.transforms.croppad.dictionary import DivisiblePadd, RandCropByPosNegLabeld
...
def load_data(t1w_csv, seg_mask_csv, age_csv_path, brain_mask_csv, batch, root_dir):
    ...
    print(
        "train set",
        len(imgs_list_train),
        len(seg_labels_train),
        len(mask_dir_train),
        len(age_labels_train),
    )
    print(
        "val set",
        len(imgs_list_val),
    ...
    filenames_train = [
        {"img": x, "seg_label": y, "age_label": z, "brain_mask": b}
        for (x, y, z, b) in zip(
            imgs_list_train, seg_labels_train, age_labels_train, mask_dir_train
        )
    ]
    ...
    filenames_val = [
        {"img": x, "seg_label": y, "age_label": z, "brain_mask": b}
        for (x, y, z, b) in zip(
            imgs_list_val, seg_labels_val, age_labels_val, mask_dir_val
        )
    ]
