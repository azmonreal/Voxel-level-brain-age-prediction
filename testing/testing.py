...
def create_pad_mask(pred, target, brain_mask):
    for i in range(len(target.cpu().numpy())):
        ...
        ground_truth = torch.mul(
            ground_truth, brain_mask[i]
        ...
        loss_img = torch.sum(
            torch.abs(torch.sub(prediction, ground_truth, alpha=1))
    ...
    with torch.no_grad():
        ...
        for step, batch in enumerate(test_loader):
            if step < 50:
                ...
                bg_ch, f_ch, s_ch, t_ch, pred_glob_age, pred_vox_age = (
                    sliding_window_inference(
                        inputs=brain_img,
                        roi_size=(128, 128, 128),
                        sw_batch_size=8,
                        predictor=model_fun1,
                        overlap=0.9,
                        progress=False,
                    )
                )
                ...
                age_output = nib.Nifti1Image(
                    age_output.cpu().numpy(), img1.affine, img1.header
                )
                ...
                img1 = nib.load(batch["img"].meta["filename_or_obj"][0])
                seg_output = nib.Nifti1Image(
                    pred_seg.cpu().numpy().squeeze(axis=0).squeeze(axis=0),
                    img1.affine,
                    img1.header,
                )
                ...
                brain_pad_mask, mae_voxel = create_pad_mask(
                    pred_vox_age, age, brain_mask
                )
                ...
                vox_age_output = nib.Nifti1Image(
                    vox_age_output.cpu().as_tensor().cpu().numpy(),
                    img1.affine,
                    img1.header,
                )
                ...
                orig_save = nib.Nifti1Image(
                    orig.cpu().numpy(), img1.affine, img1.header
                )
