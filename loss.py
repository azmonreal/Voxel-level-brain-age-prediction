...
def dice_score(pred, target):
    ...
    if intersection < 0 or union < 0 or dice > 1:
        print(
            "min, max, sum of iflat predictions ",
            torch.min(iflat).item(),
            torch.max(iflat).item(),
            iflat.sum().item(),
        )
        print(
            "min, max, sum of tflat labels ",
            torch.min(tflat).item(),
            torch.max(iflat).item(),
            iflat.sum().item(),
        )
    ...
    for i in range(len(target.cpu().numpy())):
        ...
        if torch.sum(brain_mask[i]) != 0:
            ground_truth = torch.full_like(
                pred[i], target.cpu().numpy()[i]
            )  # creates a tensor of size pred_shape and fills it with values target
            noise = torch.randint_like(ground_truth, low=-2, high=3)
            noised_ground_truth = torch.add(ground_truth, noise)
            noised_ground_truth = torch.mul(
                noised_ground_truth, brain_mask[i]
            )  # tensor with target age values only in brain region

            prediction = torch.mul(
                pred[i], brain_mask[i]
            )  # to ensure prediction tensor has non-zero values only in brain region
            loss_img = torch.sum(
                (torch.abs(prediction - noised_ground_truth))
            ) / torch.sum(brain_mask[i])  # mean absolute error voxel wise
            ...
            noise = torch.randint_like(ground_truth, low=-2, high=3)
            noised_ground_truth = torch.add(ground_truth, noise)
            dummy_brain_mask = torch.where(
                pred[i] > 0, 1, 0
            )  # to create a dummy brain mask to calc loss
            noised_ground_truth = torch.mul(
                noised_ground_truth, dummy_brain_mask
            )  # tensor with target age values only in brain region

            prediction = torch.mul(
                pred[i], dummy_brain_mask
            )  # to ensure prediction tensor has non-zero values only in brain region
            loss_img = torch.sum(
                (torch.abs(prediction - noised_ground_truth))
            ) / torch.sum(dummy_brain_mask)  # mean absolute error voxel wise
...
def overall_loss(
    seg_pred, seg_target, age_pred, age_target, root_dir, binary_mask, global_age, epoch
):
    ...
    print(
        "\nunweighted mae, global mae and dice score before loss calc ",
        mae,
        global_mae,
        dice,
    )
...
def overall_metrics(
    seg_pred, seg_target, age_pred, age_target, root_dir, binary_mask, global_age
):
    # print('calc overall metric')
    return (
        voxel_mae(age_pred, age_target, binary_mask),
        mean_channel_dice(seg_pred, seg_target),
        global_mae_loss(global_age, age_target),
    )
