import torch
from testing.testing import test_check1
import argparse
from testing.model import build_unet
from testing.data_loader import load_data_test
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--seg_mask_csv", type=str, help="path to segmentation mask csv"
    )
    parser.add_argument("--age_csv_path", type=str, help="path to age info csv")
    parser.add_argument(
        "--brain_mask_csv", type=str, help="path to binary brain mask csv"
    )
    parser.add_argument(
        "--checkpoint_path", type=str, help="path to checkpoint pth file"
    )
    ...
    ds_test, test_loader = load_data_test(
        args.t1w_csv,
        args.seg_mask_csv,
        args.age_csv_path,
        args.brain_mask_csv,
    ...
    test_check1(
        test_loader, model, optimizer, scheduler, root_dir, args.checkpoint_path
    )
